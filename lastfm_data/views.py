...
from lastfm_data.celery_tasks import fill_user_tracks
from lastfm_data.lastfm import get_albums_by_months
from lastfm_data.models import PlayedTrack, Album, ServiceUser
...
def lastfm_get_top_artists(request, username):
    ...
    if not PlayedTrack.objects.filter(user__username=username).exists():
        # Fill user tracks using celery
        result = fill_user_tracks(username)

        if not result:
            return JsonResponse({'error': "Can't get tracks from lastfm"}, status_code=400)
...
def get_forgotten_albums(request, username):
    """ Albums that user is not interested in in a structure like:
    items = {
        'forgotten_albums':
            [
                  {
                      'album_image': 'http://127.0.0.1:8000/media/album_images/
                      0a5fa3d617f246548af137271fc68ada_BVr3E2i.png',
                      'artist': 'Metallica',
                ...
                {
                    'album_image': 'http://127.0.0.1:8000/media/album_images/
                    0a40013700274522b45dc90148b665bf_bL1HqBy.png',
                      'artist': 'Murtalica',
                      'album': 'Tralala',
                      'last_listened': 2011,
                      'total_scrobbles': 50
                ...
                {
                    'album_image': 'http://127.0.0.1:8000/media/album_images/
                    3fb805c77cb645d0aaac2514e025fcfe_Z2FxHi4.png',
                      'artist': 'MickyMouse',
    ...
    for old_album in old_albums:
        ...
        tracks = PlayedTrack.objects.filter(album=old_album['album'], user__username=username).order_by(
            '-playback_date')
